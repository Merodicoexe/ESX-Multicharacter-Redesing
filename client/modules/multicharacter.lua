...
function Multicharacter:SetupCamera()
    ...
    -- Fixed camera positioning so character is properly in front of camera
    -- Camera is now positioned further back and to the side for better view
    local offset = GetOffsetFromEntityInWorldCoords(self.playerPed, 0, 2.0, 0.5)
    
    SetCamCoord(self.cam, offset.x + 0.5, offset.y, offset.z)
    PointCamAtCoord(self.cam, self.spawnCoords.x, self.spawnCoords.y, self.spawnCoords.z + 1.0)
end
