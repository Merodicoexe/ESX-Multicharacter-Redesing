...
function Database:BanCharacter(identifier, charSlot, minutes, reason, bannedBy)
    local bannedUntil = os.time() + (minutes * 60)
    
    MySQL.insert.await("INSERT INTO `character_bans` (`identifier`, `char_slot`, `banned_until`, `reason`, `banned_by`) VALUES (?, ?, ?, ?, ?)", {
        identifier,
        charSlot,
        bannedUntil,
        reason,
...
function Database:UnbanCharacter(identifier, charSlot)
    local deleted = MySQL.update.await("DELETE FROM `character_bans` WHERE `identifier` = ? AND `char_slot` = ?", {
        identifier,
        charSlot
    })
    return deleted > 0
...
function Database:GetCharacterBan(identifier, charSlot)
    local ban = MySQL.single.await("SELECT * FROM `character_bans` WHERE `identifier` = ? AND `char_slot` = ? AND `banned_until` > ?", {
        identifier,
        charSlot,
        os.time()
    })
...
function Database:GetAllBans()
    local bans = MySQL.query.await("SELECT * FROM `character_bans` WHERE `banned_until` > ? ORDER BY `banned_at` DESC", {
        os.time()
    })
    return bans or {}
end
...
function Database:CleanExpiredBans()
    MySQL.update("DELETE FROM `character_bans` WHERE `banned_until` <= ?", {
        os.time()
    })
end
