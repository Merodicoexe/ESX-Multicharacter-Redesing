    ...
    if rawCharacters then
        ...
        for i = 1, characterCount, 1 do
            ...
            if id then
                ...
                local ban = Database:GetCharacterBan(playerIdentifier, id)
                local bannedUntil = nil
...
function Multicharacter:CharacterChosen(source, charid, isNew)
    ...
    -- Check if character is banned before allowing selection
    if not isNew then
        local identifier = ESX.GetIdentifier(source)
        local ban = Database:GetCharacterBan(identifier, charid)
        
        if ban then
            local timeLeft = math.ceil((ban.banned_until - os.time()) / 60)
            TriggerClientEvent("esx:showNotification", source, string.format("Tato postava je zabanována ještě %s minut", timeLeft))
            return
        end
    end
